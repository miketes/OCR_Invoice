...
def sort_box(box):
    ...
    for i in range(0, len(li)):
        if i == len(li) - 1 or li[i] == li[i + 1]:
            ...
            op1=np.sqrt(np.sum(np.square(li[i]-li[i + 1])))
            if op1 < 20:
                vb.append([box[i],i])


    for s in vb:
        if box[s[1]][0] > box[s[1]+1][0]:
            tmp = copy.deepcopy(box[s[1]])
            box[s[1]] = box[s[1]+1]
            box[s[1] + 1] = tmp
...
#                 new_box.append(b)
#
#         print(sorted(new_box, key=lambda x: sum([x[1], x[3], x[5], x[7]])))
#         return sorted(new_box, key=lambda x: sum([x[1], x[3], x[5], x[7]]))
#
#     else:
#         print(sorted(box, key=lambda x: sum([x[1], x[3], x[5], x[7]])))
